class DayTableRepository < Hanami::Repository
  ...
  def max_date(day_tables)    
    ...
    max_date
    # (day_tables.max { |day_table| day_table.date }).date
  end

  def append_table(all, time_table, min_time)
    return if time_table.out_of_date?(min_time)
    line = all.find { |dayline| dayline[:date] == time_table.date.to_s }
    if line
      line[:table] << { time: time_table.time, remain: time_table.remain }
    else
