...
describe Web::Controllers::Project::Create do
  ...
  it 'create table without image and json' do
    response = action.call({token: Tools.make_token(1), name: 'test'})
    response[0].must_equal 422
  end

  it 'create table' do
    sleep 1
    response = action.call({token: Tools.make_token(1), name: 'test', time_state: time_state, image: "/test/test.jpg", check_mode: 'auto'})
